...
const Messages: React.FC = () => {
  ...
  useEffect(() => {
    const fetchChats = async () => {
        ...
        setUnreadCounts(convSummary);
    };
    
    fetchChats();
    // Using a simpler polling for user list is acceptable if not strictly required to be realtime, 
    // but the main requirement was for messages. 
    const interval = setInterval(fetchChats, 10000); // Relaxed polling
    return () => clearInterval(interval);
  }, [user]);
