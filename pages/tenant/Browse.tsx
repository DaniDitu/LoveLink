...
const Browse: React.FC = () => {
    ...
    // Helper to fetch users for React Query
    const fetchUsers = async ({ pageParam = null }: { pageParam?: any }) => {
        const tenantId = user?.tenantId || currentTenant?.id || 'default-tenant';
        return await db.getBrowsableUsers(tenantId, 20, pageParam);
    };

    // 1. ADS QUERY (Cached separately)
    const { data: adsData } = useQuery({
        queryKey: ['insertions', user?.tenantId || 'default'],
        queryFn: async () => {
            const tenantId = user?.tenantId || currentTenant?.id || 'default-tenant';
            return await db.getInsertions(tenantId, true);
        },
        staleTime: 1000 * 60 * 60, // 1 hour for ads
        enabled: !!user
    });

    // 2. USERS INFINITE QUERY
    const {
        data,
        fetchNextPage,
        hasNextPage,
        ...
        queryKey: ['browsableUsers', user?.tenantId || 'default'],
        queryFn: fetchUsers,
    ...
    useEffect(() => {
        if (data) {
            ...
            // Filter out current user and duplicates just in case
            setCandidates(allUsers.filter(u => u.uid !== user?.uid));
        }
    }, [data, user]);
        ...
        <div className="max-w-6xl mx-auto p-4 md:p-6 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    ...
                    <button onClick={handleRefresh} disabled={isLoading || isFetchingNextPage} className="p-2 rounded-xl border bg-white dark:bg-night-800 border-slate-200 dark:border-night-700 text-slate-500 hover:text-blue-500 transition-colors"><RefreshCw className={`w-5 h-5 ${isLoading || isFetchingNextPage ? 'animate-spin' : ''}`} /></button>
                </div>
            </div>
            ...
            <div ref={loaderRef} className="py-8 text-center">{(isLoading || isFetchingNextPage) && <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-rose-500"></div>}{!hasNextPage && candidates.length > 0 && !isLoading && !isFetchingNextPage && <p className="text-sm text-slate-400 mt-4">Hai visto tutti i profili disponibili.</p>}</div>
        </div>
    );
};
